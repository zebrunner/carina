...
public class I18N {
    ...
    public static void init() {
        ...
        List<Locale> locales = LocaleReader.init(Configuration.get(Parameter.LANGUAGE));
        ...
        for (URL u : Resources.getResourceURLs(new ResourceURLFilter() {
            ...
            LOGGER.debug(String.format("Analyzing '%s' I18N resource for loading...", u));
            ...
            if (u.getPath().endsWith("I18N.class") || u.getPath().endsWith("I18N$1.class")) {
                // separate conditions to support core JUnit tests
                continue;
            }

            if (fileName.lastIndexOf('_') == fileName.length() - 3 || fileName.lastIndexOf('_') == fileName.length() - 5) {
            ...
            String resource = filePath.substring(filePath.indexOf(SpecialKeywords.I18N)).replaceAll("/", ".") + fileName;
    ...
    public static String getText(String key, Locale locale) {
        ...
        while (iter.hasNext()) {
            ...
            try {
                ...
                LOGGER.debug("Looking for value for language:'" + locale.getLanguage() + "' current iteration language is: '"
                        + bundle.getLocale().getLanguage() + "'.");
                if (bundle.getLocale().getLanguage().equals(locale.getLanguage())) {
                    LOGGER.debug("Found language:'" + locale.getLanguage() + "' and value is '" + value + "'.");
                    return value;
                }
            } catch (MissingResourceException e) {
                // do nothing
