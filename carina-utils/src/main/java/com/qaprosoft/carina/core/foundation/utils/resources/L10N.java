...
public class L10N {
    ...
    public static void init() {
        ...
        for (URL u : Resources.getResourceURLs(new ResourceURLFilter() {
            ...
            if (u.getPath().endsWith("L10N.class") || u.getPath().endsWith("L10N$1.class")) {
            ...
            String resource = filePath.substring(filePath.indexOf(SpecialKeywords.L10N)).replaceAll("/", ".") + fileName;
    ...
    public static String getText(String key, Locale locale) {
        ...
        while (iter.hasNext()) {
            ...
            try {
                ...
                LOGGER.debug("Looking for value for locale:'" + locale.toString() + "' current iteration locale is: '" + bundle.getLocale().toString()
                        + "'.");
                if (bundle.getLocale().toString().equals(locale.toString())) {
                    LOGGER.debug("Found locale:'" + locale.toString() + "' and value is '" + value + "'.");
                    return value;
                }
            } catch (MissingResourceException e) {
                // do nothing
