...
public enum R {
    ...
    static {
        for (R resource : values()) {
            try {
                ...
                // if(CONFIG.equals(resource) &&
                // !PlaceholderResolver.isValid(properties))
                // {
                ...
                if (resource.resourceFile.contains("config.properties")) {
                    // TODO: investigate if we needed env variables analysis
                    // using System.getenv() as well
    ...
    /**
     * Returns value either from systems properties or config properties
     * context. Systems properties have higher priority. Decryption is performed
     * if required.
     * 
