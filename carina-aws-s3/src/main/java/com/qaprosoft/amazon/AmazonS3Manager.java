...
public class AmazonS3Manager {
  ...
  private static final String CRYPTO_PATTERN =
      Configuration.get(Configuration.Parameter.CRYPTO_PATTERN);
  ...
  public static synchronized AmazonS3Manager getInstance() {
    if (instance == null) {
      ...
      final String accessKey = amazonS3Manager.decryptIfEncrypted(
          Configuration.get(Configuration.Parameter.ACCESS_KEY_ID));
      final String secretKey =
          amazonS3Manager.decryptIfEncrypted(Configuration.get(Configuration.Parameter.SECRET_KEY));
  ...
  private void initCryptoTool() {
    if (this.crypto == null) {
      ...
      this.crypto = CryptoToolBuilder.builder()
          .chooseAlgorithm(
              Algorithm.find(Configuration.get(Configuration.Parameter.CRYPTO_ALGORITHM)))
          .setKey(cryptoKey)
          .build();
    }
  }
   ...
   * @param key      - S3 storage path. Example:
   ...
   *
   * @param bucket  - S3 Bucket name.
   * @param key     - S3 storage path to your project. Example:
   *                android/MyProject
   * @param pattern - pattern to find single build artifact Example:
   *                .*prod-google-release.*
    ...
    try {
      ...
      while (!appDownload.isDone()) {
        LOGGER.info("\t\t transferred: {}%",
            (int) (appDownload.getProgress().getPercentTransferred() + 0.5));
        CommonUtils.pause(pollingInterval);
      }
      LOGGER.info("\t State: {}", appDownload.getState());
    } catch (final AmazonClientException e) {
      throw new RuntimeException(
          "File wasn't downloaded from s3. See log: ".concat(e.getMessage()));
    }
  }
