...
public class AbstractTestListener extends TestArgsListener {
    ...
    private String failItem(ITestResult result, Messager messager) {
        ...
        if (!errorMessage.contains("All tests were skipped! Analyze logs to determine possible configuration issues.")) {
            ...
            if (!R.EMAIL.getBoolean("fail_full_stacktrace_in_report") && result.getThrowable() != null && result.getThrowable().getMessage() != null
                    && !StringUtils.isEmpty(result.getThrowable().getMessage())) {
                EmailReportItemCollector
                        .push(createTestResult(result, TestResultType.FAIL, result.getThrowable().getMessage(), result.getMethod().getDescription()));
            } else {
    ...
    private String skipItem(ITestResult result, Messager messager) {
        ...
        if (errorMessage.isEmpty()) {
            // identify is it due to the dependent failure or exception in
            // before suite/class/method
            ...
            if (dependentMethod) {
                ...
                // Try to find error details from last configuration failure in
                // this thread
    ...
    public void beforeConfiguration(ITestResult result) {
        ...
        // // This context cleanup is required to launch dependent steps if
        // parent method pass from Nth retry!
        removeIncorrectlyFailedTests(result.getTestContext());

        // added 3 below lines to be able to track log/screenshots for before
        // suite/class/method actions too
        TestNamingUtil.releaseTestInfoByThread();
    ...
    public void onStart(ITestContext context) {
        ...
        ReportContext.getBaseDir(); // create directory for logging as soon as
                                    // possible

        /*
         * //dropbox client initialization if
         * (!Configuration.get(Parameter.DROPBOX_ACCESS_TOKEN).isEmpty()) {
         * dropboxClient = new
         * DropboxClient(Configuration.get(Parameter.DROPBOX_ACCESS_TOKEN)); }
         */
    ...
    public void onTestStart(ITestResult result) {
        ...
        if (!result.getTestContext().getCurrentXmlTest().getTestParameters().containsKey(SpecialKeywords.EXCEL_DS_CUSTOM_PROVIDER)
                && result.getParameters().length > 0) // set parameters from XLS
                                                      // only if test contains
                                                      // any parameter at
                                                      // all)
        {
        ...
        // obligatory reset any registered canonical name because for
        // ALREADY_PASSED methods we can't do this in
    ...
    private void removeIncorrectlyFailedTests(ITestContext context) {
        ...
        for (ITestResult passedTest : context.getPassedTests().getAllResults()) {
            ...
            // LOGGER.debug("Adding passedTest info: " + passedTestId + "; " +
            // passedTest.getName());
        ...
        for (ITestResult failedTest : context.getFailedTests().getAllResults()) {
            ...
            if (failedTestIds.contains(failedTestId) || passedTestIds.contains(failedTestId)) {
                // LOGGER.debug("Test to be removed from context: " +
                // failedTestId + "; " + failedTest.getName());
                testsToBeRemoved.add(failedTest);
            } else {
                // LOGGER.debug("Test to mark as failed: " + failedTestId + "; "
                // + failedTest.getName());
        ...
        for (Iterator<ITestResult> iterator = context.getFailedTests().getAllResults().iterator(); iterator.hasNext();) {
            ...
            if (testsToBeRemoved.contains(testResult)) {
                // LOGGER.debug("Removing test from context: " +
                // testResult.getName());
                iterator.remove();
            }
        }
    ...
    private long getMethodId(ITestResult result) {
        ...
        // LOGGER.debug("Calculated id for " +
        // result.getMethod().getMethodName() + " is " + id);
        return id;
    }
    ...
    protected TestResultItem createTestResult(ITestResult result, TestResultType resultType, String failReason, String description) {
        ...
        // String linkToScreenshots =
        // ReportContext.getTestScreenshotsLink(testName);
        ...
        if (!FileUtils.listFiles(ReportContext.getTestDir(), new String[] { "png" }, false).isEmpty()) {
            if (TestResultType.PASS.equals(resultType) && !Configuration.getBoolean(Parameter.KEEP_ALL_SCREENSHOTS)) {
                // remove physically all screenshots if test/config pass and
                // KEEP_ALL_SCREENSHOTS=false to improve
        ...
        // AUTO-1081 eTAF report does not show linked Jira tickets if test
        // PASSED
        // jira tickets should be used for tracking tasks. application issues
        // will be tracked by planned zafira feature
    ...
    private String takeScreenshot(ITestResult result, String msg) {
        ...
        for (Map.Entry<String, WebDriver> entry : drivers.entrySet()) {
            ...
            screenId = Screenshot.captureFullSize(entry.getValue(), driverName + ": " + msg); // in
                                                                                              // case
                                                                                              // of
                                                                                              // failure
        }
