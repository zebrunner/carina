...
public class HtmlReportGenerator {
    ...
    private static synchronized void createReportAsHTML(File testFolder, List<File> images) {
        try {
            ...
            // String wholeReport =
            // R.REPORT.get("container").replace("${images}",
            // report.toString());
    ...
    private static void copyGalleryLib() {
        ...
        if (!new File(reportsRootDir.getAbsolutePath() + "/gallery-lib").exists()) {
            try {
                ...
                ZipManager.copyInputStream(is, new BufferedOutputStream(new FileOutputStream(reportsRootDir.getAbsolutePath() + "/" + GALLERY_ZIP)));
