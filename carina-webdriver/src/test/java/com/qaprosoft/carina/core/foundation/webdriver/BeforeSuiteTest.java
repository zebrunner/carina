...
public class BeforeSuiteTest implements IDriverPool {
	...
	@Test(dependsOnMethods = {"beforeSuiteDriverSuccessfulRegistered"}, expectedExceptions = {AssertionError.class }, expectedExceptionsMessageRegExp = "Driver 'default' is already registered for thread: 1")
	...
	public void beforeClassBehaviourNewDriverName() {
		...
		Assert.assertEquals(getDriver(BEFOR_CLASS_DRIVER_NAME), mockBeforeClassDriver, "Incorrect driver has been returned");
	}
	
	@Test(dependsOnMethods = {"beforeClassBehaviourNewDriverName"}, expectedExceptions = {AssertionError.class }, expectedExceptionsMessageRegExp = "Driver 'default' is already registered for thread: 1")
	...
	public void beforeMethodBehaviourNewDriverName() {
		...
		Assert.assertEquals(getDriver(BEFOR_METHOD_DRIVER_NAME), mockBeforeMethodDriver, "Incorrect driver has been returned");
	}

	@Test(dependsOnMethods = {"beforeMethodBehaviourNewDriverName"}, expectedExceptions = {AssertionError.class }, expectedExceptionsMessageRegExp = "Driver 'default' is already registered for thread: 1")
