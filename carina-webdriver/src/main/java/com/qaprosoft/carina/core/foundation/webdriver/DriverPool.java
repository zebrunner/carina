...
public final class DriverPool {
    ...
    public static WebDriver getDriver(String name, DesiredCapabilities capabilities, String seleniumHost) {
        ...
        if (currentDrivers.containsKey(name)) {
            ...
            drv = single_driver;
        } else if ((driverMode == DriverMode.CLASS_MODE || driverMode == DriverMode.METHOD_MODE) && Configuration.getInt(Parameter.THREAD_COUNT) == 1
            ...
            LOGGER.debug("Try to find driver by ThreadGroup id values! Current ThreadGroup count is: " + threads.length);
    ...
    public static void quitDriver(String name) {
        ...
        // TODO: try to understand valid place for recorded video file to
        // support method/class and suite mode
     ...
     * @param capabilities DesiredCapabilities
     * @param seleniumHost String
     * @param device Device where we want to start driver
     * @return WebDriver
     */
        ...
        while (!init & count++ < maxCount) {
            try {
                ...
                if (!device.isNull()) {
                    ...
                    // there is no sense to clean cache and reinstall app if we
                    // request dedicated device
                    device.screenOn();

                    device.restartAppium();
                    device.clearAppData();

                    // verify if valid build is already installed and uninstall
                ...
                DevicePool.deregisterDevice();
                LOGGER.error(String.format("Driver initialization '%s' FAILED for selenium: %s! Retry %d of %d time - %s", name, seleniumHost, count,
                        maxCount, thr.getMessage()), thr);
                init_throwable = thr;
    ...
    // https://github.com/lightbody/browsermob-proxy/issues/264 'started' flag
    // is not set to false after stopping BrowserMobProxyServer
    // Due to the above issue we can't control BrowserMob isRunning state and
    // shouldn't stop it
     ...
     * 
     * LOGGER.debug("stopProxy starting..."); if (proxies.containsKey(threadId))
     * { BrowserMobProxy proxy = proxies.get(threadId); if (proxy != null) {
     * LOGGER.debug("Found registered proxy by thread: " + threadId);
     * 
     * if (proxy.isStarted()) { LOGGER.info("Stopping BrowserMob proxy...");
     * proxy.stop(); } else {
     * LOGGER.info("Stopping BrowserMob proxy skipped as it is not started."); }
     * } proxies.remove(threadId); } LOGGER.debug("stopProxy finished..."); }
     */
