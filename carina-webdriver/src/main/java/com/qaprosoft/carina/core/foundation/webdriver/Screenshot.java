...
public class Screenshot {
    ...
    // Collects screenshot comments. Screenshot comments are associated using
     ...
     * 
     * @param driver instance used for capturing.
     * @param isTakeScreenshot perform actual capture or not
     * @return screenshot name.
     */
    @Deprecated
    ...
    /**
     * Captures driver screenshot for Alice-AI metadata and put it to
     * appropriate metadata location
     * 
     * @param driver instance used for capturing.
    ...
    private static String capture(WebDriver driver, boolean isTakeScreenshot, String comment, boolean fullSize) {
        ...
        if (isTakeScreenshot && !DriverFactory.HTML_UNIT.equalsIgnoreCase(Configuration.get(Parameter.BROWSER))) {
            ...
            try {
                ...
                // TODO: implement naming strategy for screenshots wihtout test
                // names
                // String fileID = test.replaceAll("\\W+", "_") + "-" +
                ...
                if (Configuration.getInt(Parameter.BIG_SCREEN_WIDTH) != -1 && Configuration.getInt(Parameter.BIG_SCREEN_HEIGHT) != -1) {
                    resizeImg(screen, Configuration.getInt(Parameter.BIG_SCREEN_WIDTH), Configuration.getInt(Parameter.BIG_SCREEN_HEIGHT),
                            screenPath);
                }
                ...
                // TODO: Move upload to S3 into the async calls closer to the
                // end of test to upload whole bundle
                // Also investigate possibility to remove rederence onto the
                // test name
                String test = "";
     ...
     * 
     * @param bufImage - image to resize.
    ...
    private static BufferedImage takeFullScreenshot(WebDriver driver, WebDriver augmentedDriver) throws IOException {
        ...
        if (driver.getClass().toString().contains("java_client") || Configuration.getDriverType().equals(SpecialKeywords.MOBILE)) {
            File screenshot = ((AppiumDriver<?>) driver).getScreenshotAs(OutputType.FILE);
            screenShot = ImageIO.read(screenshot);
        } else {
            ru.yandex.qatools.ashot.Screenshot screenshot = new AShot().shootingStrategy(ShootingStrategies.viewportPasting(100))
    ...
    private static WebDriver getAugmentedDriver() {
        AppiumDriver<?> augmentedDriver = ((AppiumDriver<?>) DriverPool.getDriver());
        augmentedDriver = (AppiumDriver<?>) new Augmenter().augment(augmentedDriver);
        return augmentedDriver;
    }

    /*
     * Makes visible screenshot and saves it to user's directory
     * 
     * @param driver - webDriver.
     * 
     * @param location - directory to save (if location ="" screenshot isn't
     * saved!)
     * 
     * @exception IOException
     * 
