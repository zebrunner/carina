...
public class Device {
    ...
    public String toString() {
        return String.format("name: %s; type: %s; os: %s; osVersion: %s; udid: %s; selenium: %s; remoteURL: %s", name, type, os, osVersion, udid,
                seleniumServer, remoteURL);
    }
    ...
    public void connectRemote() {
        ...
        // TODO: add several attempt of connect until device appear among
        // connected devices
    ...
    private Boolean getScreenState() {
        ...
        String[] cmd = CmdLine.insertCommandsAfter(executor.getDefaultCmd(), "-s", getAdbName(), "shell", "dumpsys", "input_method");
        ...
        if (screenState == null) {
            LOGGER.error(udid + ": Unable to determine existing device screen state!");
            return screenState; // no actions required if state is not
                                // recognized.
        }
    ...
    public void screenOff() {
        ...
        if (screenState) {
            String[] cmd = CmdLine.insertCommandsAfter(executor.getDefaultCmd(), "-s", getAdbName(), "shell", "input", "keyevent", "26");
            executor.execute(cmd);
    ...
    public void pressKey(int key) {
        ...
        String[] cmd = CmdLine.insertCommandsAfter(executor.getDefaultCmd(), "-s", getAdbName(), "shell", "input", "keyevent", String.valueOf(key));
        executor.execute(cmd);
    }
    ...
    public String getApkPackageName(String apkFile) {
        ...
        // output:
        // package: name='com.myfitnesspal.android' versionCode='9025'
        // versionName='develop-QA' platformBuildVersionName='6.0-2704002'
        ...
        if (Configuration.getBoolean(Parameter.MOBILE_APP_UNINSTALL)) {
            ...
            if (apkVersions != null) {
                ...
                if (apkVersions[1].equals(apkInstalledVersions[1]) && apkVersions[2].equals(apkInstalledVersions[2]) && oldMobileApp.isEmpty()) {
                    LOGGER.info("Skip application uninstall and cache cleanup as exactly the same version is already installed.");
                } else {
                    ...
                    if (Configuration.getBoolean(Parameter.MOBILE_APP_INSTALL)) {
                        // install application in single thread to fix issue
                        // with gray Google maps
    ...
    // TODO: think about moving shutdown/startup scripts into external property
