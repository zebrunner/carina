...
public class ExtendedFieldDecorator implements FieldDecorator {
    ...
    public Object decorate(ClassLoader loader, Field field) {
        if ((!field.isAnnotationPresent(FindBy.class) && !field.isAnnotationPresent(FindByAI.class))
                /*
                 * Enable field decorator logic only in case of presence the
                 * FindBy/FindByAI annotation in the field
                 */ || !(ExtendedWebElement.class.isAssignableFrom(field.getType()) || AbstractUIObject.class.isAssignableFrom(field.getType())
                        || isDecoratableList(
                                field)) /*
                                         ...
                                         * ExtendedWebElement or derived from
                                         * AbstractUIObject or DecoratableList
                                         */) {
    ...
    protected <T extends AbstractUIObject> T proxyForAbstractUIObject(ClassLoader loader, Field field, ElementLocator locator) {
        ...
        try {
            ...
            LOGGER.error("Implement appropriate AbstractUIObject constructor for auto-initialization: " + e.getMessage());
    ...
    protected <T extends AbstractUIObject> List<T> proxyForListUIObjects(ClassLoader loader, Field field, ElementLocator locator) {
        InvocationHandler handler = new AbstractUIObjectListHandler<T>((Class<?>) getListType(field), webDriver, locator, field.getName());
