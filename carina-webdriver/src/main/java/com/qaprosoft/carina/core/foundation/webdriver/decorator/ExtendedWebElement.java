...
//      Wait<WebDriver> wait = new WebDriverWait(getDriver(), timeout, RETRY_TIME)
        ...
        Wait<WebDriver> wait = new WebDriverWait((WebDriver) searchContext, java.time.Clock.tickMillis(java.time.ZoneId.systemDefault()), Sleeper.SYSTEM_SLEEPER, timeout, RETRY_TIME)
...
//				ExpectedConditions.textToBe(getBy(), decryptedText)), timeout);
    ...
    public boolean waitUntilElementDisappear(final long timeout) {
        ...
        return waitUntil(ExpectedConditions.or(ExpectedConditions.invisibilityOfElementLocated(by),
