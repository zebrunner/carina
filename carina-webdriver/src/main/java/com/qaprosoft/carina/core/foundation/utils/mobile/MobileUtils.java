...
public class MobileUtils {
    ...
    public static boolean swipeInDevice(ExtendedWebElement element, Direction direction,
                                        double minCoefficient, double maxCoefficient, int duration) {
        ...
        switch (direction) {
            case LEFT:
                startx = (int) (elementLocation.getX() + Math
                        .round(maxCoefficient
                                * elementDimensions.getWidth()));
                starty = elementLocation.getY()
                        + Math.round(elementDimensions.getHeight() / 2);
                endx = (int) (elementLocation.getX() + Math
                        .round(minCoefficient
                                * elementDimensions.getWidth()));
                endy = starty;
                break;
            case RIGHT:
                ...
                break;
            case UP:
                startx = elementLocation.getX()
                        + Math.round(elementDimensions.getWidth() / 2);
                starty = (int) (elementLocation.getY() + Math
                        .round(maxCoefficient
                                * elementDimensions.getHeight()));
                endx = startx;
                endy = (int) (elementLocation.getY() + Math
                        .round(minCoefficient
                                * elementDimensions.getHeight()));
                break;
            case DOWN:
        ...
        LOGGER.debug(String.format(
                "Swipe from (X = %d; Y = %d) to (X = %d; Y = %d)", startx,
                starty, endx, endy));
        try {
            ...
            LOGGER.error(String.format(
                    "Error during Swipe from (X = %d; Y = %d) to (X = %d; Y = %d): %s", startx,
                    starty, endx, endy, e));
        }
        return false;
    }
     ...
     * @return boolean
     */
    public static boolean swipeInContainerTillElementWithStartDirection(ExtendedWebElement element, ExtendedWebElement container, Direction direction ) {
        int swipeTimes = 10;
        boolean isPresent = element.isElementPresent(2);
        LOGGER.info("Swipe to element: ".concat(element.getNameWithLocator().toString()));
    ...
    public static boolean swipeUntilElementPresence(final ExtendedWebElement element, int times, int duration) {
        ...
        while (!isPresent && times-- > 0) {
            ...
            x = scrSize.width / 2;
            // swipe coordinates for ios should be less than for android to avoid elements' skipping
